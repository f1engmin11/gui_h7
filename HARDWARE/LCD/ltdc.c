...
//根据不同的颜色格式,定义帧缓存数组
// 一共1280*800个数组元素，每个元素对应一个像素，每个像素占用两个字节，即u16
u16 ltdc_lcd_framebuf[1280][800] __attribute__((at(LCD_FRAME_BUF_ADDR)));	//定义最大屏分辨率时,LCD所需的帧缓存数组大小,u16对应RGB565
...
void LTDC_Init(void)
{
	...
	if(lcdid==0X1018)				//10.1寸1280*800 RGB屏	
	{
		...
		LTDC_Clk_Set(5, 160, 16);		//设置像素时钟 25/5 * 160 / 16 = 50MHz
		//LTDC_Clk_Set(1, 6, 3);		//设置像素时钟
	}
...
void HAL_LTDC_MspInit(LTDC_HandleTypeDef* hltdc)
{
	...
	//初始化PG6,7,11，LCD_R7，LCD_CLK，LCD_B3
	GPIO_Initure.Pin=GPIO_PIN_6|GPIO_PIN_7|GPIO_PIN_11;
	HAL_GPIO_Init(GPIOG,&GPIO_Initure);

	//初始化PH9,10,11,12,13,14,15，LCD_R3~LCD_R6，LCD_G2~LCD_G4
	GPIO_Initure.Pin=GPIO_PIN_9|GPIO_PIN_10|GPIO_PIN_11|\
					GPIO_PIN_12|GPIO_PIN_13|GPIO_PIN_14|GPIO_PIN_15;
	HAL_GPIO_Init(GPIOH,&GPIO_Initure);

	//初始化PI0,1,2,4,5,6,7,9,10，LCD_G5~LCD_G7，LCD_B4~LCD_B7，LCD_VSYNC，LCD_HSYNC
