...
{								 
	...
	u16 outtime=0;
 	cnt=0;				
	POINT_COLOR=BLUE_LCD;
	BACK_COLOR =WHITE_LCD;
	LCD_Clear(WHITE_LCD);//清屏   
	POINT_COLOR=RED_LCD;//红色 
	LCD_Clear(WHITE_LCD);//清屏 	   
	POINT_COLOR=BLACK_LCD;
	LCD_ShowString(40,40,160,100,16,(u8*)TP_REMIND_MSG_TBL);//显示提示信息
	TP_Drow_Touch_Point(20,20,RED_LCD);//画点1 
	tp_dev.sta=0;//消除触发信号 
	...
	while(1)//如果连续10秒钟没有按下,则自动退出
	{
		...
		{	
			...
			{			   
				case 1:						 
					TP_Drow_Touch_Point(20,20,WHITE_LCD);				//清除点1 
					TP_Drow_Touch_Point(lcddev.width-20,20,RED_LCD);	//画点2
					break;
				case 2:
 					TP_Drow_Touch_Point(lcddev.width-20,20,WHITE_LCD);	//清除点2
					TP_Drow_Touch_Point(20,lcddev.height-20,RED_LCD);	//画点3
					break;
				case 3:
 					TP_Drow_Touch_Point(20,lcddev.height-20,WHITE_LCD);			//清除点3
 					TP_Drow_Touch_Point(lcddev.width-20,lcddev.height-20,RED_LCD);	//画点4
					break;
				case 4:	 //全部四个点已经得到
	    		    //对边相等
					...
					{
						cnt=0;
 				    	TP_Drow_Touch_Point(lcddev.width-20,lcddev.height-20,WHITE_LCD);	//清除点4
   	 					TP_Drow_Touch_Point(20,20,RED_LCD);								//画点1
